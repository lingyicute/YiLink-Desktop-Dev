...
public class UpdateService
{
    ...
    public async Task CheckUpdateGuiN(Config config, Action<bool, string> updateFunc, bool preRelease)
    {
        ...
        _updateFunc?.Invoke(false, string.Format(ResUI.MsgStartUpdating, ECoreType.YiLink));
        var result = await CheckUpdateAsync(downloadHandle, ECoreType.YiLink, preRelease);
        if (result.Success)
        {
            _updateFunc?.Invoke(false, string.Format(ResUI.MsgParsingSuccessfully, ECoreType.YiLink));
    ...
    private async Task<string?> GetUrlFromCore(CoreInfo? coreInfo)
    {
        if (Utils.IsWindows())
        {
            ...
            if (coreInfo?.CoreType != ECoreType.YiLink)
            {
                return url;
            }
