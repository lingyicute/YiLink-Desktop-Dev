...
public class CheckUpdateViewModel : MyReactiveObject
{
    ...
    private string _YiLink = ECoreType.YiLink.ToString();
    ...
    private void RefreshCheckUpdateItems()
    {
        ...
        {
            _checkUpdateModel.Add(GetCheckUpdateModel(_YiLink));
    ...
    private async Task CheckUpdateN(bool preRelease)
    {
        void _updateUI(bool success, string msg)
        {
            UpdateView(_YiLink, msg);
            if (success)
            {
                UpdateView(_YiLink, ResUI.OperationSuccess);
                UpdatedPlusPlus(_YiLink, msg);
            }
        }
        await (new UpdateService()).CheckUpdateGuiN(_config, _updateUI, preRelease)
            .ContinueWith(t =>
            {
                UpdatedPlusPlus(_YiLink, "");
            });
    }
    ...
    private async Task UpdateFinished()
    {
        if (_lstUpdated.Count > 0 && _lstUpdated.Count(x => x.IsFinished == true) == _lstUpdated.Count)
        {
            ...
            if (_lstUpdated.Any(x => x.CoreType == _YiLink && x.IsFinished == true))
            {
    ...
    private void UpgradeN()
    {
        try
        {
            var fileName = _lstUpdated.FirstOrDefault(x => x.CoreType == _YiLink)?.FileName;
            if (fileName.IsNullOrEmpty())
            {
                return;
            }
            ...
            {
                UpdateView(_YiLink, ResUI.UpgradeAppNotExistTip);
                return;
            }
